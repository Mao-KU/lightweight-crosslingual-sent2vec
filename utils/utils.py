...
def save_checkpoint(state, is_best, epoch):
    filename = os.path.join(config.output_dir , config.model_name ,"checkpoint_{}.pth.tar".format(str(epoch)))
    torch.save(state, filename)
    logger.info('Saving weights to %s'%filename)
    if is_best:
        message = os.path.join(config.output_dir , config.model_name, config.best_models, 'model_best.pth.tar')
